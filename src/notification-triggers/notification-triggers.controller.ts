...
export class NotificationTriggersController {
  ...
  @ApiOperation({ summary: 'List all notification copy overrides for an event' })
  ...
  @Put(':eventId')
  @Roles(Role.TIKIT_ADMIN, Role.KARORDFORANDE)
  @ApiOperation({ summary: 'Create or update a notification copy override for an event' })
  upsert(
  ...
  @ApiQuery({ name: 'schoolId', required: false, type: String, description: 'TIKIT_ADMIN only' })
  deleteOne(
    @Param('eventId') eventId: string,
    ...
    return this.notificationTriggersService.deleteOne(schoolId, eventId, triggerKey);
  }
}
